using System;
using System.Collections.Generic;
using System.Text;

namespace Bonsai.RazorPages.User.Services.LanguageDictionary
{
    public class UserLanguageDictionary
    {
        public const string RegisterHeading = "User pages - Register - Heading";
        public const string RegisterButton = "User pages - Register - Button";

        public const string LoginHeading = "User pages - Login - Heading";
        public const string LoginButton = "User pages - Login - Button";

        public const string Email = "User pages - Email";
        public const string EmailPlaceholder = "User pages - Email - Placeholder";
        public const string EmailFormatError = "User pages - Email - Error - Not email";
        public const string EmailNotEmpty = "User pages - Email - Error - Empty";


        public const string Password = "User pages - Password";
        public const string PasswordNotEmpty = "User pages - Password - Error - Empty";

        public const string PasswordConfirm = "User pages - Confirm password";
        public const string PasswordConfirmSame = "User pages - Confirm password - Error - Must be same";

        public const string DeniedHeading = "User pages - Denied - Heading";
        public const string DeniedText = "User pages - Denied - Text";

        public const string ReturnButton = "User pages - Return button";
        public const string WebButton = "User pages - Web button";

        public const string ChangePasswordHeading = "User pages - Change password";
        public const string ChangePasswordButton = "User pages - Change password - Button";
        public const string OldPassword = "User pages - Old password";
        public const string OldPasswordNotEmpty = "User pages - Old password - Empty";
        public const string NewPasswordLabel = "User pages - New password";
        public const string NewPasswordNotEmpty = "User pages - New password - Empty";
        public const string ConfirmNewPasswordLabel = "User pages - Confirm new password";
        public const string ConfirmNewPasswordSame = "User pages - Confirm new password - Same";

        public const string LogoutSuccessHeading = "User pages - Logout success";
        public const string LogoutSuccessText = "User pages - Logout success - Text";

        
            
    }
}
